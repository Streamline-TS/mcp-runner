...
use mcp_runner::transport::json_rpc::{JsonRpcResponse, JSON_RPC_VERSION};
use serde_json::json;
...
async fn test_tool_response_as_jsonrpc() {
    ...
    assert!(received_message.is_ok(), "Should receive a message within timeout");
    ...
    assert_eq!(message.id, Some(request_id.to_string()), "Message ID should match request ID");
    
    // Parse the data field as a JSON-RPC response
    let json_rpc: JsonRpcResponse = serde_json::from_str(&message.data).expect("Data should be a valid JSON-RPC response");
    
    // Verify the JSON-RPC response structure
    assert_eq!(json_rpc.jsonrpc, JSON_RPC_VERSION, "JSON-RPC version should match");
    assert_eq!(json_rpc.id, json!(request_id), "JSON-RPC ID should match request ID");
    assert!(json_rpc.result.is_some(), "Result should be present");
    assert_eq!(json_rpc.result.unwrap(), response_data, "Result data should match");
    assert!(json_rpc.error.is_none(), "Error should not be present in a success response");
}
...
async fn test_tool_error_as_jsonrpc() {
    ...
    assert_eq!(json_rpc.jsonrpc, JSON_RPC_VERSION, "JSON-RPC version should match");
    assert_eq!(json_rpc.id, json!(request_id), "JSON-RPC ID should match request ID");
    assert!(json_rpc.result.is_none(), "Result should not be present in an error response");
    ...
    assert_eq!(error_data["serverId"], json!(server_id), "Server ID should match");
    assert_eq!(error_data["toolName"], json!(tool_name), "Tool name should match");
}
...
async fn test_server_status_event() {
    ...
    assert_eq!(received.id, None, "Server status events should not have an ID");
...
async fn test_notification_event() {
    ...
    assert_eq!(received.id, None, "Notification events should not have an ID");
