...
async fn test_call_tool() -> Result<()> {
    ...
    mock_transport
        .expect_call_tool()
        .with(eq("echo"), 
              eq(serde_json::json!({"message": "hello"})))
        .times(1)
        .returning(|_, _| {
            Ok(serde_json::json!({
    ...
    // Execute the test
    let input = TestInput { message: "hello".to_string() };
    let output: TestOutput = client.call_tool("echo", &input).await?;
    
    // Verify the result
    assert_eq!(output, TestOutput { response: "hello from echo".to_string() });
...
async fn test_list_resources() -> Result<()> {
    ...
    assert_eq!(resources[0].description, Some("A test resource".to_string()));
...
async fn test_serialization_error() -> Result<()> {
    ...
    let result = client.call_tool::<_, HashMap<String, String>>("test", &Unserializable).await;
    
    assert!(result.is_err());
    if let Err(e) = result {
        assert!(e.to_string().contains("serialization"), 
                "Expected serialization error, got: {}", e);
    }
...
async fn test_deserialization_error() -> Result<()> {
    ...
    assert!(result.is_err());
    if let Err(e) = result {
        assert!(e.to_string().contains("deserialize"), 
                "Expected deserialization error, got: {}", e);
    }
...
async fn test_transport_error_propagation() -> Result<()> {
    ...
    mock_transport
        .expect_list_tools()
        .returning(|| {
            Err(Error::Communication("Transport error".to_string()))
        });
