...
impl MockStdio {
    ...
    fn read_line(&mut self) -> io::Result<String> {
        if let Some(line) = self.recv_buffer.pop() {
            ...
            Err(io::Error::new(io::ErrorKind::UnexpectedEof, "No more mock input"))
        }
    }
...
impl Transport for TestableStdioTransport {
    ...
    async fn list_tools(&self) -> Result<Vec<Value>> {
        ...
        Err(Error::JsonRpc("Invalid listTools response format".to_string()))
    }
    ...
    async fn call_tool(&self, name: &str, args: Value) -> Result<Value> {
        ...
        Err(Error::JsonRpc("Invalid callTool response format".to_string()))
    }
    ...
    async fn list_resources(&self) -> Result<Vec<Value>> {
        ...
        Err(Error::JsonRpc("Invalid listResources response format".to_string()))
    }
    ...
    async fn get_resource(&self, uri: &str) -> Result<Value> {
        ...
        Err(Error::JsonRpc("Invalid getResource response format".to_string()))
    }
}
...
async fn test_call_tool_request_format() {
    // Prepare mock response
    let mock_responses = vec![
        r#"{"jsonrpc":"2.0","id":2,"result":{"output":"success"}}"#.to_string(),
    ];
    ...
    // Call the method
    let _ = transport.call_tool("test-tool", json!({"param": "value"})).await;
