...
fn test_parse_config() -> Result<()> {
    ...
    assert_eq!(fs_config.args, vec!["-y", "@modelcontextprotocol/server-filesystem", "/path/to/files"]);
    assert!(fs_config.env.is_empty());
    
    let gh_config = &config.mcp_servers["github"];
    assert_eq!(gh_config.command, "npx");
    assert_eq!(gh_config.args, vec!["-y", "@modelcontextprotocol/server-github"]);
    assert_eq!(gh_config.env.get("GITHUB_TOKEN"), Some(&"your_token_here".to_string()));
...
fn test_validate_config() -> Result<()> {
    ...
    let fs_config = ServerConfig {
        ...
        args: vec!["-y".to_string(), "@modelcontextprotocol/server-filesystem".to_string(), "/path".to_string()],
        env: HashMap::new(),
    };
